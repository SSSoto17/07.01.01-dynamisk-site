...
function showProduct(product) {
  ...
  clonedProduct.querySelector("img").src =
    "https://kea-alt-del.dk/t7/images/webp/640/" + product.id + ".webp";

  clonedProduct.querySelector(".item_overview h1").textContent =
    product.productdisplayname;
  clonedProduct.querySelector(".item_overview h2").textContent =
    product.brandname;

  clonedProduct.querySelector(".item_overview .color").textContent =
    product.color1;

  if (product.discount) {
    clonedProduct.querySelector("h3.before_discount span").textContent =
      product.price;
    clonedProduct.querySelector("h3.price span").textContent = Math.round(
      product.price * (product.discount / 100)
    );
    clonedProduct.querySelector("p.discount_percentage span").textContent =
      product.discount;
  } else {
  ...
  clonedProduct.querySelector(".item_description h3").innerHTML =
    product.styledesc;
  clonedProduct.querySelector(".item_description p.desc").innerHTML =
    product.description;
  clonedProduct.querySelector(".item_description p.care_desc").innerHTML =
